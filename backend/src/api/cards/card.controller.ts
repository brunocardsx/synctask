...
import { z } from 'zod';
import { moveCardSchema, createCardSchema, updateCardSchema } from '../../schemas/cardSchema.js';
...
export const createCard = async (req: Request, res: Response) => {
    try {
        ...
        const card = await cardService.createCard(title, description || '', columnId, userId);

        if (!card) {
            return res.status(404).json({ message: 'Column not found or you do not have permission to create cards.' });
        }
...
export const updateCard = async (req: Request, res: Response) => {
    try {
        ...
        const updatedCard = await cardService.updateCard(cardId, title, description || '', userId);
