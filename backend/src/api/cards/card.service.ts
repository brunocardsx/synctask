...
export const createCard = async (
  ...
  return await prisma.$transaction(async tx => {
    ...
    console.log(
      `📡 Evento 'card:created' emitido para board ${column.boardId}`
    );
...
export const updateCard = async (
  ...
  return await prisma.$transaction(async tx => {
    ...
    console.log(
      `📡 Evento 'card:updated' emitido para board ${card.column.boardId}`
    );
...
export const deleteCard = async (cardId: string, userId: string) => {
  return await prisma.$transaction(async tx => {
    ...
    getIO()
      .to(boardId)
      .emit('card:deleted', { cardId, columnId: card.columnId });
