...
export const updateColumn = async (columnId: string, title: string, ownerId: string) => {
    const column = await prisma.column.findUnique({
        where: { id: columnId },
        include: { board: true }
    });
    ...
    console.log(`📡 Evento 'column:updated' emitido para board ${column.boardId}`);
...
export const deleteColumn = async (columnId: string, ownerId: string) => {
    ...
    console.log(`📡 Evento 'column:deleted' emitido para board ${column.boardId}`);

    return { success: true };
};

export const reorderColumn = async (columnId: string, newOrder: number, ownerId: string) => {
    const column = await prisma.column.findUnique({
        where: { id: columnId },
        include: { board: true }
    });
